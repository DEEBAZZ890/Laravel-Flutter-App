                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    @else
                        <table class="table w-full">
                            <caption>{{ __('All Recommendations (paginated)') }}</caption>
                            <thead class="border border-stone-300">
                            <tr class="bg-stone-300">
                                <th class="p-2 text-right">#</th>
                                <th class="p-2 text-left">{{ __("Result ID") }}</th>
                                <th class="p-2 text-left">{{ __("Content") }}</th>
                                <th class="p-2 text-left">{{ __("Action Link") }}</th>
                            ...
                            @foreach($recommendations as $recommendation)
                                <tr class="border-b border-stone-300 hover:bg-stone-200 transition duration-500 ease-in-out">
                                    ...
                                    <td class="p-2">{{ $recommendation->result_id }}</td>
                                    <td class="p-2">{{ Str::limit($recommendation->content, 50) }}</td>
                                    <td class="p-2">{{ $recommendation->action_link }}</td>
                                    <td class="py-2 pl-2 pr-0 flex flex-row gap-2">
                                        <a href="{{ route('recommendations.show', ['recommendation' => $recommendation->id]) }}"
                                        ...
                                        <a href="{{ route('recommendations.edit', ['recommendation' => $recommendation->id]) }}"
                                        ...
                                        <a href="{{ route('recommendations.delete', ['recommendation' => $recommendation->id]) }}"
