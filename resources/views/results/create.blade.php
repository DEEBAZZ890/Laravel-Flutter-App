<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Add Result') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h3 class="font-bold text-gray-700 text-lg mb-4">
                        {{ __("Add New Result") }}
                    </h3>

                    @if ($errors->any())
                        <div class="bg-red-200 text-red-800 p-2 rounded border-red-800 mb-4">
                            <i class="fa fa-exclamation-triangle text-xl pl-2 pr-4"></i>
                            You have errors in your form submission.
                        </div>
                    @endif

                    <form action="{{ route('results.store') }}" method="post" class="flex flex-col gap-4">
                        @csrf

                        <!-- Quiz Attempt ID Field -->
                        <div class="grid grid-cols-6 gap-1">
                            <label for="quiz_attempt_id" class="">{{ __("Quiz Attempt ID") }}</label>
                            <input id="quiz_attempt_id" name="quiz_attempt_id" type="number"
                                   class="p-2 col-span-5"
                                   value="{{ old('quiz_attempt_id') }}">
                            @error('quiz_attempt_id')
                            <p class="text-red-800 mb-2 text-sm col-span-5">{{ $message }}</p>
                            @enderror
                        </div>

                        <!-- User ID Field -->
                        <div class="grid grid-cols-6 gap-1">
                            <label for="user_id" class="">{{ __("User ID") }}</label>
                            <input id="user_id" name="user_id" type="number"
                                   class="p-2 col-span-5"
                                   value="{{ old('user_id') }}">
                            @error('user_id')
                            <p class="text-red-800 mb-2 text-sm col-span-5">{{ $message }}</p>
                            @enderror
                        </div>

                        <!-- Score Field -->
                        <div class="grid grid-cols-6 gap-1">
                            <label for="score" class="">{{ __("Score") }}</label>
                            <input id="score" name="score" type="number"
                                   class="p-2 col-span-5"
                                   value="{{ old('score') }}">
                            @error('score')
                            <p class="text-red-800 mb-2 text-sm col-span-5">{{ $message }}</p>
                            @enderror
                        </div>

                        <!-- Passed Field -->
                        <div class="grid grid-cols-6 gap-1">
                            <label for="passed" class="">{{ __("Passed") }}</label>
                            <select id="passed" name="passed" class="p-2 col-span-5">
                                <option value="1" {{ old('passed') == '1' ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ old('passed') == '0' ? 'selected' : '' }}>No</option>
                            </select>
                            @error('passed')
                            <p class="text-red-800 mb-2 text-sm col-span-5">{{ $message }}</p>
                            @enderror
                        </div>

                        <!-- Feedback Field -->
                        <div class="grid grid-cols-6 gap-1">
                            <label for="feedback" class="">{{ __("Feedback") }}</label>
                            <textarea id="feedback" name="feedback"
                                      class="p-2 col-span-5">{{ old('feedback') }}</textarea>
                            @error('feedback')
                            <p class="text-red-800 mb-2 text-sm col-span-5">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="grid grid-cols-6 gap-4">
                            <span></span>
                            <div class="mt-6 col-span-5 flex flex-row gap-4 -ml-2">
                                <a href="{{ route('results.index') }}"
                                   class="py-2 px-4 mx-2 w-1/6 text-center
                                      rounded border border-stone-600
                                      hover:bg-stone-600
                                      text-stone-600 hover:text-white
                                      transition duration-500">
                                    <i class="fa fa-circle-left"></i> {{ __("Back") }}
                                </a>

                                <button type="submit"
                                        class="py-2 px-4 mx-2 w-1/6 text-center
                                       rounded border border-red-600
                                       hover:bg-red-600
                                       text-red-600 hover:text-white
                                       transition duration-500">
                                    <i class="fa fa-save"></i> {{ __("Save") }}
                                </button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>
